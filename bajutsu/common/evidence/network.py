...
class ControlChannel(Protocol):
    ...
    because its receiver already holds the queue; the driver-observed collectors do not, and
    `orchestrator.control_channel` turns a command issued against one of those into a loud failure

...
- [x] グループ 3、作業単位 23。常駐チャネルは、装飾ウィンドウを取り除くために自分が
  ...
  の取得が要求したときにだけ行います。
- [ ] グループ 3、作業単位 17——2 回の読み取りの間にアクセシビリティイベントが発火して
  いなければ `settledDump` の 2 回目を省く。実装しましたが、計測の結果、取り下げました。
  前提自体は単体では正しいものです。ノードの bounds はアクセシビリティイベントを経て
  ダンプに届くので、イベントがなければ変化もありません。見落としていたのは、その確認用の
  ダンプが**サーバー自身のリスナーへイベントを流し続ける役割も担っている**ことでした。
  ホストの read-lag バリアが解放の根拠にしているのは、そのリスナーが保つ read mark です。
  2 回目のダンプをやめると mark が止まります。API 34 のエミュレータで `controls.yaml` の
  scroll ステップは 7.5 秒、7.7 秒、10.0 秒になりました。ダンプを戻すと 5.7 秒、5.9 秒、
  6.0 秒です。伸びない mark を相手に、pan のバリアが予算を使い切っていたためです。
  着地には、ダンプへ依存せず mark を生かし続ける方法が要ります。
- [ ] グループ 3、作業単位 20——常駐サーバーの `UiAutomation.takeScreenshot()` で
  スクリーンショットを撮る。実装していません。この設計は作業単位 2 より前のもので、
  作業単位 2 は前日に着地し（[#1925](https://github.com/bajutsu-e2e/bajutsu/pull/1925)）、
  `adb exec-out screencap` のサブプロセスをツリー読み取りと重ねて臨界パスの外へ
  すでに出しています。撮影を常駐チャネルへ移すと、その重なりを深めるどころか壊します。
  サーバーは一度に 1 本の接続しか処理しないので、いま並走している読み取りの後ろに
  直列化されるからです。作業単位 2 はこの組を、直列で 3361 ミリ秒、重ねて 2182 ミリ秒と
  計測しています。この作業単位が狙うサブプロセス起動は 103 ミリ秒です。並行処理する
  サーバーが入口になりますが、`UiAutomation` がダンプと並行するスクリーンショットを
  許すかは文書化されていません。性能ではなく決定性の問題なので、ここでは推測しません。
- [ ] グループ 3、作業単位 24——`/act` に swipe 版を追加する。2 通り実装し、いずれも
  実機計測の結果、取り下げました。`UiDevice.swipe` を使うと `controls.yaml` の scroll
  ステップは 12.8 秒になり、座標経路の 4.2 秒に対して大幅に遅くなりました。UiAutomator は
  ドラッグを `UiAutomation.executeAndWaitForEvent` 経由で刻んでおり、この API は見えた
  イベントを消費します。pan のイベントはすべてドラッグの**最中**に発生するので、read mark
  には 1 つも届かず、応答は publish を確認できず、ホストのバリアが 4 秒を使い切りました。
  ドラッグを `MotionEvent` から組み立て直すと 7.5 秒まで下がりました。同じ理由で
  `injectDoubleTap` がすでに採っている形です。それでも座標経路より遅く、サーバー側に
  仕込んだプローブは、pan の全区間で read mark が動いていないことを示しました。
  そのドラッグはコンテンツも動かしていなかったことになります。この作業単位が挙げていた根本原因は
...
- [x] 各グループの出荷後に、`controls.yaml` に対して
  ...
  arm64 エミュレータ、`controls.yaml`、前後それぞれ 3 回）。壁時計はトレーサーではなく
  実行マニフェストのステップ時間から取りました。この項目の目標がステップ単位で述べられて
  いることと、エミュレータ自身のばらつきが呼び出し単位の削減を覆い隠すためです。`tap`
  ステップは 3.26〜5.44 秒から 2.00〜2.36 秒へ、`scroll` ステップは 3.93〜6.01 秒から
  4.22〜5.92 秒で、ばらつきの範囲内で変わりません。tap のおよそ 3 分の 1 を削った形で、
...
- PR — グループ 3、作業単位 16、18、19、21、22、23。リクエスト自身より新しいイベントを端末に
  ...
  すべて成功しました。前後それぞれ 3 回の計測で、`tap` ステップは 3.26〜5.44 秒から 2.00〜2.36 秒へ

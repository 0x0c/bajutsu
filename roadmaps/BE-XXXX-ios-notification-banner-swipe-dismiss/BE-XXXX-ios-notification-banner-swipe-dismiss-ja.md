...
4つ目は、3つ目で monitor が呼ばれると分かった場合に限り、handler が `true` を返す前に、その内側で
発行したスワイプによって通知バナーが消えたことを handler 自身が確認できるかどうかです。XCUITest が
割り込みの解消を検証するのは handler が返った後であり、そこで通知バナーがまだ出ていれば monitor を
再度呼び出します。BE-0399 自身の動機は、monitor が
...
この2つの経路は同じ重みを持ちません。無条件で選んでよいのは一方だけです。handler は、`true` を
返す前に、その内側で発行したスワイプによって通知バナーが消えたことを自ら確認できます。それを
Unit 1 が確かめた場合に限り、ガードはその monitor 経由で応答します。これは BE-0399 の monitor が
割り込んでくるアラートに応答する形と同じです。それ以外の
すべての場合、つまり Unit 1 がその確認を取れなかった場合も含め、ガードは代わりに各アクションステップ
自身の操作の直前にポーリングして消す方式を使います。この操作直前の確認は、上限付き間隔のポーリングが
最後に返した答えを使い回さず、ステップの境界で自前の存在照会を発行します。記憶した照会結果に基づいて
動くことこそ、BE-0399 が測定した不具合だからです。この代替方式には、認めた上での制約が残ります。
...
`systemAlertHandling` とその `--system-alert-handling` CLI フラグがすでに確立している config から
シナリオへ、さらにフラグで上書きできる優先順位
...
通知バナーの枠の内側に置きます。そうすることで、重なりがタイミング頼みではなく構成として成立します。
該当する showcase のコントロールがなければ、この Unit で追加します。実機の通知バナーに対するネイティブ
...
[`docs/scenarios.md`](../../docs/scenarios.md)、その CLI フラグを扱う
[`docs/cli.md`](../../docs/cli.md)、およびそれぞれの `docs/ja/` 対訳に、新しいスイッチを `interrupts`
...
- [ ] Unit 5 — showcase の fixture（アプリ側のフォアグラウンドでの通知バナー表示と、通知バナーの枠の
      内側に置くタップ先を含む）と実機検証。

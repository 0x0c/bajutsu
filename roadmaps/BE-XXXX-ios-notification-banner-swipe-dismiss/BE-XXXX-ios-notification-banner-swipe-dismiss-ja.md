...
重なることがあります。通知バナーには、シナリオが確認を置ける決まったきっかけがありません。プッシュ
通知の到着はサーバー側に依存し、アプリ自身がスケジュールするローカル通知であっても、どのステップの
直後かではなく実時刻を基準に届きます。そのためタップ先がたまたま通知バナーの枠と重なっていると、
...
本提案が実現したかどうかは、後から読む人が次の方法で確かめられます。シナリオの途中でローカル通知を
発火させ、通知バナーの実測された枠がシナリオの次のタップ先と重なるように調整したシナリオを実行します。
本提案の前は、そのタップの成否は当てになりません。XCUITest が解決する瞬間に2つの枠がどう重なっているか
次第だからです。本提案の後は、タップが発火する前に通知バナーが自ら消え、タップは毎回タップ先に届きます。
...
iOS バージョン（18.6、26.3、26.4、26.5）です。次の3点が、以降のすべての Unit の設計を左右します。
...
この Unit はコードを生み出しません。この測定が、Unit 2 から Unit 4 まで残っている設計上の疑問に
決着をつけます。
...
報告するだけで何も判定しない、薄い非ブロッキングの読み取りです。
...
すでに持つ座標計算をそのまま再利用し、新しいジェスチャーのプリミティブは追加しません。

### Unit 4 — リアクティブな組み込み

config とシナリオの双方で設定できるスイッチが、Unit 2 の存在照会を BE-0315 が SpringBoard の照会に
対してすでに確立した上限付き間隔でポーリングするガードを有効にし、通知バナーを見つけた瞬間に Unit 3
のアクションで消します。Unit 1 で、重なった通知バナーに対して XCUITest 自身の interruption monitor が
システムアラートと同じ扱いをすることが分かった場合は、ガードもその monitor 経由で応答します。これは
BE-0399 の monitor が割り込んでくるアラートに応答する形と同じです。Unit 1 でそうならないと分かった
場合は、各アクションステップ自身の操作の直前にポーリングして消す方式を代わりに使います。このスイッチは、
`dismissAlerts` がすでに確立している config からシナリオへ、さらにフラグで上書きできる優先順位
（[BE-0177](../BE-0177-run-behavior-target-config/BE-0177-run-behavior-target-config-ja.md)）に従います。

### Unit 5 — showcase の fixture と実機検証

シナリオの途中でローカル通知を発火させ、通知バナーの実測された枠がシナリオの次のタップ先と重なる
ように調整し、タップがそのタップ先に届くことをアサートする showcase シナリオを追加します。実機の
通知バナーに対するネイティブなスワイプは、Simulator を使わないゲートでは証明できません。driver の
メソッドを実装する Unit は、このシナリオを起動済みの Simulator 上で実行しなければなりません。

### Unit 6 — ドキュメント

[`docs/scenarios.md`](../../docs/scenarios.md) とその `docs/ja/` 対訳に、新しいスイッチを `interrupts`
や `dismissAlerts`/`systemAlertHandling` と並べて記載し、どの仕組みを使うべきかという BE-0314 が
すでに持っている比較を拡張します。

### Unit 7 — テスト

新しいスイッチのスキーマの parse/validate、ポーリングのたびに存在照会の返り値が入れ替わる fake driver、
ステップ自身の操作より前にガードが通知バナーを消すこと、config からシナリオへの優先順位、そして
Unit 1 で interruption monitor 経由の経路が必要だと分かった場合は、BE-0399 のテストスイートが

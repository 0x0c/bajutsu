...
iOS バージョン（18.6、26.3、26.4、26.5）です。次の5点が、以降のすべての Unit の設計を左右します。
...
再度呼び出します。5つ目は、本提案とは独立した論点で、後段でまとめて述べます。
...
handler に渡り、default handler はアラートの既定ボタンを押して解消します。通知バナーにボタンはなく、
その handler が押せるものはありません。そのため、3つ目の測定で通知バナーが monitor まで届くと分かった
場合、5つ目の測定は、これが本提案のコードとは無関係に、BE-0399 自身が測定した再呼び出しループを今日
すでに再現しているかどうかです。解消を確認できないまま割り込みを引き受けた monitor は、以降のあらゆる
...
ではなく不在を返します。通知バナーが同時に2つ以上表示されている場合、この照会は実測した枠に基づいて
最も手前のバナーを1件だけ返します。これは、XCUITest がたまたま先に返したバナーではなく、明確に定めた
決定論的な順序です（prime directive 2）。これにより Unit 4 のガードは、そのバナーを消してから残りを
再ポーリングでき、消せる割り込みのためにランを失敗させずに済みます。

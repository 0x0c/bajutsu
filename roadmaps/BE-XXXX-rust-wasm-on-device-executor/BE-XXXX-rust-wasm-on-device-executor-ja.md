...
スイートのフィクスチャ集合に対して、両方のビルドが同じ結果を返すことを確認します。検証すべき
性質は、テストで裏付けたということだけでなく、構造としてズレが起こらないということです。
...
ホストのPyO3バインディングと各デバイスのWASMバイナリが同じクレートからビルドされるため、
セレクタの変更がホストとデバイスの間でズレることは、もう起こりません。同じコンパイル済み
ロジックが両方の場所で動くからです。
[BE-0114](../BE-0114-driver-conformance-suite/BE-0114-driver-conformance-suite-ja.md)の
ドライバ適合スイートは、`FakeDriver`や他のバックエンドと同じフィクスチャに対してこの
クレートを引き続き検証します。ただし、2つの独立したデバイス側移植の間のズレを検知する
必要はなくなります。実装が1つしか存在しないためです。
...
トークンで保護し、クレートのステップループがステップの境界ごとにポーリングします。これは
[BE-0370](../BE-0370-graceful-run-cancel/BE-0370-graceful-run-cancel-ja.md)が
すでに前提としている反応の粒度と同じです。したがって`SIGTERM`やWeb UIの停止ボタンは、今日と
...
- [ ] PyO3バインディングと新しい`Capability`をビルドし、この`Capability`を宣言しない
  ドライバの挙動を変えないまま、ホスト側の判定再計算の経路を`run_scenario`へ配線します。

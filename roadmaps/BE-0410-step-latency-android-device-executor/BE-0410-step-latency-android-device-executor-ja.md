...
[BE-0408](../BE-0408-step-latency-device-executor-protocol/BE-0408-step-latency-device-executor-protocol-ja.md) は、端末側ステップ実行プロトコルを定義しています。何をホスト側から
...
調整の [BE-0407](../BE-0407-step-latency-driver-internal-tuning/BE-0407-step-latency-driver-internal-tuning-ja.md) はすでに、実測した `POST /act` の平均 2204 ミリ秒のほとんどを、
...
ドライバ内部調整の [BE-0407](../BE-0407-step-latency-driver-internal-tuning/BE-0407-step-latency-driver-internal-tuning-ja.md)、
端末側プロトコルの BE-0408、iOS 実行機の
[BE-0409](../BE-0409-step-latency-ios-device-executor/BE-0409-step-latency-ios-device-executor-ja.md)、
この項目の順に進みます。**この項目への着手は、BE-0409 が完了するまで行っては
いけません。**
...
2. **`wait` と `settled` を、`TYPE_WINDOW_CONTENT_CHANGED` と `WINDOWS_CHANGED`
   ...
   置き換えます。これは、ドライバ内部調整の BE-0407 が `POSTDATE_BUDGET_MS` の
...
- **完全なイベント駆動の実行機を構築せず、BE-0407 が提案する
...
**順番の状態：BE-0409 の完了待ちです**（詳細設計の「実装の順番」を参照）。

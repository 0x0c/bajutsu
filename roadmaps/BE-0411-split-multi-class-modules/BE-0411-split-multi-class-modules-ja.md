...
- PR_LOG_PLACEHOLDER — 全バッチを一度に実施しました。まず`scripts/split_modules.py`と
  ...
  バッチの前に直しました。自己レビューで見つかった7件は、この後にまとめます。
  ...
  完成した差分に対する自己レビューで、スクリプトにさらに7件の欠陥が見つかりました。いずれも同じ形で、
  ...
  のは、これが原因です。現在はいずれも拒否または修正となり、整形処理・削除処理・バッチループのどれも、
  終了コード0ではなく失敗を報告します。テストは41件から75件に増え、代表的な3つのプランを実際の

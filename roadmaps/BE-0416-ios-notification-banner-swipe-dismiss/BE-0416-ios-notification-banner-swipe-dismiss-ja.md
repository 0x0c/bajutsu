...
iOS バージョン（18.6、26.3、26.4、26.5）です。この測定からは6つの問いへの答えが得られます。そのうち
5つが、以降のすべての Unit の設計を左右します。

1. 通知バナーをどのプロセスの要素ツリーが公開しているか(システムアラートと同じく SpringBoard なのか、
   別のプロセスなのか)。
2. 通知バナーがどのような枠や識別子を提供するか、そもそも提供するか。
3. シナリオが通知バナーを2件同時に発生させたときに照会が何を返すか。個別に測定できる枠なのか、
   順序付けの手がかりがないまま重なる枠なのか。
4. タップ先が通知バナーの枠の下にある通常の `tap`/`type` ステップが、BE-0399 がシステムアラートに
   ついて測定したのと同じ XCUITest 自身の interruption monitor の扱いをすでに受けているか、それとも
   ヒットテストが割り込みを一切発生させずに重なりを静かに解決してしまうか。
5. 4つ目で monitor が呼ばれると分かった場合に限り、handler が `true` を返す前に、その内側で発行した
   スワイプによって通知バナーが消えたことを handler 自身が確認できるか。XCUITest が割り込みの解消を
   検証するのは handler が返った後であり、そこで通知バナーがまだ出ていれば monitor を再度呼び出します。

6つ目の問いは、本提案とは独立しています。次に述べます。

BE-0399 の monitor は、`systemAlertHandling` を有効にしたあらゆる run ですでにインストールされて
います。monitor がマッチできない割り込みを decline すると、そのアラートは XCUITest 純正の default
handler に渡ります。default handler はアラートの既定ボタンを押して解消しますが、通知バナーにボタン
はなく、その handler が押せるものはありません。そのため、4つ目の測定で通知バナーが monitor まで
届くと分かった場合、6つ目の問いは、これが本提案のコードとは無関係に、BE-0399 自身が測定した再呼び出し
ループを今日すでに再現しているかどうかです。解消を確認できないまま割り込みを引き受けた monitor は、以降のあらゆる
操作のたびに再度呼び出され、BE-0399 が測定したすべての試行でランナーが落ちるまでループしました。もし
そこでループが確認されれば、それは本項目が生んだものではない既存の不具合であり、本項目の可否とは別に
それ自体を報告すべきです。この Unit はコードを生み出しません。この測定が、Unit 4 に残っている設計上の
疑問に決着をつけます。
...
ではなく不在を返します。通知バナーが同時に2つ以上表示されている場合の挙動は、Unit 1 の3つ目の測定に
従います。iOS の通知バナーは通常、複数あっても同じ画面位置に積み重なるため、照会がどのバナー要素を
たまたま列挙したかに関わらず、共有された枠がそのままスワイプすべき正しい対象を示します。ガードはその
枠を消してから再ポーリングし、残りがあれば同じ手順を繰り返します。これは Unit 3 と Unit 4 がすでに
述べている、1ポーリングにつき1件ずつ消す流れと同じです。Unit 1 で、同時に発生する通知バナーが実際
には重ならない別々の枠を持つと分かった場合は、その枠に基づいて最も手前のバナーを1件だけ返します。
これは、XCUITest がたまたま先に返したバナーではなく、明確に定めた決定論的な順序です（prime
directive 2）。これにより Unit 4 のガードは、そのバナーを消してから残りを再ポーリングでき、消せる
割り込みのためにランを失敗させずに済みます。
...
消す操作と同じ上方向で、画面の外へ出す終点ではなく、通知バナーの枠より上にある画面内の1点を終点とする
スワイプです。このアクションは、`swipe` の既存の driver 実装がすでに持つ座標計算をそのまま再利用し、
新しいジェスチャーのプリミティブは追加しません。この計算はアプリ自身の原点からのオフセットとして
1点を解決するため、この Unit では、Unit 2 が返す枠(通知バナーを所有するプロセスの座標系で測定)を
その座標系にどう変換するかも記述します。Unit 2 は常に
1件以下(複数重なっていれば最も手前の1件)しか返さないため、このアクションは常に受け取った1つの枠
だけを消します。2件以上の通知バナーが重なっているときは、ガード側が1ポーリングにつき1件ずつ消して
いきます。
...
のアクションで消します。ガードがどちらの経路を取るかは、Unit 1 の5つ目の測定が決めます。ただし、
...
割り込んでくるアラートに応答する形と同じです。この monitor 経由の経路は、`systemAlertHandling` も
有効になっている場合に限り使えます。これは BE-0399 がインストールした唯一のグローバルな monitor で
あり、`policy.governs` によって制限されているためです。`policy.governs` を立てるのは
`systemAlertHandling` だけです。`systemAlertHandling` が無効なら、通知バナー用のスイッチはこの
monitor に一切届かず、Unit 1 の測定結果に関わらず常に以下のフォールバックを使います。それ以外の
すべての場合、つまり Unit 1 がその確認を取れなかった場合も含め、ガードは代わりに各アクションステップ
自身の操作の直前にポーリングして消し、さらに自らのスワイプの後にもう一度存在照会を発行して、通知
バナーが消えたと報告されるまでステップのタップを発火させません。スワイプは瞬時には終わらず、ドラッグが
指を離した瞬間にタップを合成すると、消滅アニメーションがまだ続いている通知バナーにタップが当たり
得るからです。この操作直前の確認は、上限付き間隔のポーリングが最後に返した答えを使い回さず、ステップの
境界で自前の存在照会を発行します。記憶した照会結果に基づいて動くことこそ、BE-0399 が測定した不具合だから
です。この代替方式には、認めた上での制約が残ります。

...
showcase のスイートでは、`id: search.field` が 9 ファイルにまたがって 25 回、`id: log.submit` が
...
ではなく読み込みエラーです。理由は第二原則です。併合を許すなら、名前付きセレクタがすでに
...
解決は `bajutsu/common/scenario/expand.py` の新しいパスで、`expand_components` より**前**に走ります。
そのため component のステップも `ref` を使えますし、`${params.*}` の束縛がソースに書かれていない
名前をでっち上げることはできません。このパスの後、`ref` は 1 つも残りません。
...
1. **辞書ファイルとそのモデル** — 名前から `Selector` へのマッピング、キー重複の拒否、
   `<語幹>.<キー>` の命名。
2. **`selectors:` のファイルレベルキー** — `contained_ref` による解決と、スキーマの繰り上げ。
3. **`Selector.ref`** と排他のバリデータ。未知の名前と連鎖した項目に対する読み込みエラー。
4. **展開パス** — component の展開より前に置き、端末を使わないローダーと `run` のローダーの両方へ。
5. **Author エディタの拒否**とそのメッセージ。
6. **showcase スイートの移行** — 繰り返された識別子を辞書へ移します。この項目の検証可能な帰結でも
   あります。
7. **ドキュメント** — 文法リファレンス、シナリオのガイド、およびそれぞれの日本語版。

### 第一原則との整合

- **実行経路に LLM を入れない。** 展開はテキスト上の決定的な処理で、実行ループより前に起きます。
- **決定性。** `ref` は何とも併合しないので、名前はちょうど 1 つのセレクタに解決し、覚えるべき
  優先規則がありません。解決と曖昧性の扱いは変わりません。ランナーが見るのは今日と同じ
  `Selector` です。
- **アプリ非依存。** 辞書はシナリオスイートのファイルなので、アプリケーションごとの識別子は

...
こちらはすでに任意であり、すでに機能を落として動きます。サーバーがビルドされていないとき、
Android 環境は座標だけの経路にフォールバックします。つまり pip インストールでも Android は今日
すでに、忠実度を下げた形で動きます。サーバーのビルド成果物はアーキテクチャに依存しないデータ
...
パッケージインデックスのプロジェクトページを 54 KB の日本語設計文書に向けています。ここは README
...
1. **バージョンの単一情報源化。** `dynamic = ["version"]`、`[tool.hatch.version]`、
   `bajutsu/__init__.py` の `0.1.0`、そしてタグと `__version__` のガード。
2. **`bajutsu --version`。** ルートのコールバックと [cli](../../docs/ja/cli.md) への記載。
3. **リリースワークフロー。** タグ起動、`make check`、`uv build`、信頼された公開、生成される
   リリースノート、そして PR 上のドライランジョブ。
4. **公開後の検証。** AI を含まない基本インストールと、自己参照の各 extra を、まっさらな環境で
   確かめます。
5. **パッケージングのメタデータ。** README を絶対リンクで指す readme、`[project.urls]`、分類子。
6. **XCUITest ランナーの経路。** 併走する配布物、`ios` extra、`doctor` の Xcode 互換性検査。
7. **UI Automator サーバー。** パッケージデータへの同梱と、リリースワークフローの Gradle 手順。
8. **ドキュメント。** 両言語の README と入門ガイドのインストール対応表、および `ai-boundary` の
   裏づけの一文。

### 第一原則との整合

- **実行経路に LLM を入れない。** ここで扱うのはパッケージングとリリース工程だけで、ランナーにも
  合否判定にも触れません。
- **決定性。** リリースはどのブランチにも課している `make check` で門番され、公開後の検査は作業
  ツリーではなく成果物そのものを確かめます。
- **アプリ非依存。** インストール対応表はアプリケーションではなくバックエンドで区分し、`ios`

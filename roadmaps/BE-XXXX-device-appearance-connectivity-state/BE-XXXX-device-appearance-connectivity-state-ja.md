...
### web バックエンドが初めて `DeviceControl` を持ちます
...
追加すると、要素ツリーにもネットワークにも解決しない唯一のアサーションになります。この 2 つが
[selectors](../../docs/ja/selectors.md) の記す軸です。外観をまたぐ組み合わせは、新しい文法なしに
合成から出てきます。
...
- **外観のアサーション種別を追加する。** 却下します。端末の設定を読むだけで、`setAppearance` の直後
  ではほぼ同語反復であり、アプリについて何も言いません。要素ツリーにもネットワークにも解決しない、
  文法で唯一のアサーションにもなります。そして作成者が求める主張は、外観ごとのベースラインに対する
  `visual` ですでに書けます。使うはずだった読み戻しは、ランナーの落ち着き条件として残します。

...
頼らず指せることです。いまの文法はその 2 つしか用意していません。2 つ目は、`audit` が
...
いくつもの呼び出し箇所がこの性質に依存しています。`base.default_wait_for`
（`base.py:965`）は `len(find_all(driver.query(), sel)) >= 1` であり、各バックエンドの `wait_for`
はここへ委譲します。起点がないときに送出してしまうと、
...
（`assertions/evaluate.py:85`）と、`equals: 0` を伴う `_eval_count`（`evaluate.py:124`）です。
一致しない `within` のコンテナは、すでに空のリストを返します（`base.py:783`）。これは新しい契約
ではなく、既存の契約です。

**複数に一致する起点は、その場で失敗せずに候補集合を広げます。** 広がりは黙って進みません。増えた
候補はそのまま `resolve_unique` に流れ、いつもどおり `AmbiguousSelector` が送出されます。声を
上げる失敗は 1 段あとに、それを本来担っている場所で起きます。`resolve_unique` のメッセージは、
位置関係フィールドと、集合を広げた起点を名指しします
（`the 'below:' anchor {id: form.emailLabel} matched 2 elements` のように書きます）。保守する人を
対象要素のほうへ向かわせずに済みます。
...
3. **解決**（`base.find_all`）です。フィルタの鎖とフロンティアを実装し、`resolve_unique` には
   触れません。`find_all` が全域のままであることを、依存する呼び出し箇所で証明します。
...
- 複数に一致した起点に、専用の診断を与えるべきかどうかです。いまは候補集合が広がり、通常の
  `AmbiguousSelector` として表に出ます。正直ではありますが、2 つの原因を一緒くたにします。対象
  そのものが曖昧な場合と、起点が曖昧だった場合です。

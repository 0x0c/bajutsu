...
### Wording-only findings are fixed like any other (BE-0343, retired)

Two of the reviewer's lenses judge nothing but wording: Japanese prose quality, and its English
`docs/*.md`/roadmap-prose counterpart. A finding from either carries the same plain `(non-blocking)`
decoration every other finding does, and a `suggestion` block wherever the fix is mechanical. Fix it
the way you fix any other review comment: push the rewrite to the same pull request that raised it,
then reply and resolve the thread, under the rules above.

BE-0343 used to route such a finding through an automated companion pull request instead — a second
branch and pull request, opened against your own, that shipped the fix without re-running your pull
request's CI. A later item retired that automation: a companion pull request is a second pull
request every contributor has to separately notice, review, and merge, and that standing cost
outweighed the occasional CI cycle it was built to save.
...
reason. Strike through the unit's own description, and nest the reason underneath as a sub-bullet.
The strikethrough marks a unit closed without code shipped under this item, not a unit still open

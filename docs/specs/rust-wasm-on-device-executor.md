...
書く。ホスト(Python、PyO3経由)とデバイス上のランナー(iOS/Android、WASM経由)の両方が、同じバイナリの
判定ロジックを呼ぶ。これにより、ステップ実行のホスト・デバイス間往復を除去する。BE-0408〜0410が提案する
段階的ロールアウトの代替案であり、どちらを採るかの比較材料として並記する。

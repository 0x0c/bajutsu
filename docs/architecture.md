...
- DSL `systemAlertHandling` (BE-0315), the reactive counterpart: an alert guard that fires only
  ...
  against the same rule a previous round already dismissed, that round declines to tap it at all —
  ...
  it. That match is keyed on the rule's own shape rather than the buttons the probe reads, since

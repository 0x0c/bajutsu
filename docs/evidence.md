...
pin does. That is why `--touch-markers` leaves a scenario with no `visual` verdict unmarked when
that effective environment already sets `BAJUTSU_CONTROL_CHANNEL=1`: adding the marker key there

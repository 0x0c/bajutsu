...
| **アプリバイナリ** | `appPath` をディスク上に保持。存在しないときだけ build | アップロードされたバンドルのバイナリは presigned URL で worker に届き、build は走りません（BE-0413）。Git 由来の config は引き続き worker がチェックアウトから build します。リモート build はゲート付き（BE-0121） |

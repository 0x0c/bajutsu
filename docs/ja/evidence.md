...
全シナリオに固定している場合も、実際に起動したアプリが受け取る値のとおりに判定します。起動環境に
`BAJUTSU_CONTROL_CHANNEL=1` を自分で置いたシナリオが、スクリーンショットを読む判定を持たないことも
あります。その組み合わせでは、`--touch-markers` はマーカーの値を与えません。マーカーの値を足すと

...
観測するのでこのループ自体がありません。両方がそろったときにかぎって、`--touch-markers` は
判定がスクリーンショットを読むシナリオのチャネルを起動します。それ以外では、スクリーンショットを
比較するシナリオもこのチャネルが登場する前と同じ挙動のままで、マーカーを描きません。その分だけ、
調査者はそのシナリオのタッチの証跡を失います。このチャネルが取り除こうとしている、まさにその
トレードオフを、運べる範囲でだけ払っていることになります。

チャネルが動く場合、アプリ側では二重に閉じられており、2 つ目はビルド設定です。`BajutsuKit` は

...
- **あるジェスチャの跡は、次のジェスチャが始まるまで残ります。** タイマーで消える仕組みは
  ...
  有効にした run のスクリーンショットは無効の run と異なるものになります。`visual` アサーションは
  マーカーを隠した状態で自分の撮影を得るため（後述）、フラグと画素の比較は同じ run に同居できます。
...
ほかの種別はアクセシビリティツリー、通信の記録、クリップボードのいずれかを読みます。どちらのレーンの
アプリも、今日時点では `-DBAJUTSU_ENABLE_CONTROL_CHANNEL` を付けてビルドしていません。そのため、
どちらかに `visual` シナリオを加えると、ビルドにこの設定が入るまで失敗します（後述）。
...
BajutsuKit のポーリングは実際に動作する Simulator プロセスの内側で動くため、`fake` では誰も
引き取らない collector が立つだけになり、`adb` / `playwright` はそもそも別の方法でネットワークを
観測するのでこのループ自体がありません。`xcuitest` actuator とネットワーク収集の両方がそろった
ときにかぎって、`--touch-markers` は判定がスクリーンショットを読むシナリオのチャネルを起動します。
それ以外では、スクリーンショットを
...
自分で置いた値なのかを見分けられません。起動環境に `BAJUTSU_CONTROL_CHANNEL=1` を自分で置いた
シナリオが、スクリーンショットを読む判定を持たないこともあります。その組み合わせでは、
`--touch-markers` はマーカーの値を与えません。マーカーの値を足すと 2 つの値がそろってしまうためです。
ベースライン画像のディレクトリがある run で `expect` を走らせるシナリオなら、求めていないチャネルに
入って失敗します。マーカーを描かないという扱いになり、run はその旨を標準エラー出力に書きます。

...
コードを出荷せずに閉じたユニットも、`実装済み` の項目では全チェックのチェックリストに含めます。
調査したうえで対応しないと判断したユニットも、後続の項目に切り出してそちらが出荷したユニットも、
これに当てはまります。後者の場合は、切り出し先の項目を理由の中で名指しします。そのユニットの
チェックには印を付け、ユニット自身の説明を取り消し線で消し、理由をその下に入れ子のサブバレットとして
書き添えます。取り消し線は、この項目自身のコードとしては出荷していないことを示すもので、対応が
まだ残っていることを示すものではありません

...
`実装中`）に変え、対応する `進捗` のチェックを付け、その PR を `実装 PR` に記録します。あるユニットを
調査したうえで対応しないと判断したときも、`実装済み` の項目はすべてのチェックが付いたチェックリストを
保ちます。そのユニットのチェックには印を付け、ユニット自身の説明を取り消し線で消し、理由をその下に
入れ子の箇条書きとして書き添えます。取り消し線は、コードの出荷ではなく判断によってそのユニットを
閉じたことを示します（[BE-0407](../../roadmaps/BE-0407-step-latency-driver-internal-tuning/BE-0407-step-latency-driver-internal-tuning-ja.md)、

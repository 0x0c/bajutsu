...
### 文章表現だけの指摘も、ほかの指摘と同じ扱いです（BE-0343 は廃止済み）

レビュアーのレンズのうち 2 つは、文章表現だけを見ます。日本語の文章の質と、対応する英語
（`docs/*.md` とロードマップの散文）の文章の質です。どちらのレンズから出た指摘にも、ほかの指摘と
同じ、素の `(non-blocking)` という装飾を付けます。修正が機械的に決まる場合は `suggestion` ブロック
も添えます。直し方もほかのレビューコメントと同じです。その指摘を出した pull request へ書き換えを
push し、上の規範のとおり返信してスレッドを解決します。

BE-0343 は、こうした指摘を自動の companion pull request で届けていました。あなたのブランチを
base とする別のブランチと pull request を開き、あなたの pull request 自体の CI を再実行させずに
修正を届ける仕組みです。後続の項目が、この自動化を廃止しました。companion pull request は、
貢献者が自分の取り組みとは別に、気づいてレビューしてマージしなければならない、もう 1 つの
pull request です。その標準装備のコストが、ときおり発生する CI 一式のコストを上回ったための廃止
です。
...
`実装中`）に変え、対応する `進捗` のチェックを付け、その PR を `実装 PR` に記録します。この項目自身の
コードを出荷せずに閉じたユニットも、`実装済み` の項目ではすべてのチェックが付いたチェックリストに
含めます。調査したうえで対応しないと判断したユニットも、後続の項目に切り出してそちらが出荷した
ユニットも、これに当てはまります。後者の場合は、切り出し先の項目を理由の中で名指しします。その
ユニットのチェックには印を付け、ユニット自身の説明を取り消し線で消し、理由をその下に入れ子の
箇条書きとして書き添えます。取り消し線は、この項目自身のコードとしては出荷していないことを示す
もので、対応がまだ残っていることを示すものではありません
（[BE-0407](../../roadmaps/BE-0407-step-latency-driver-internal-tuning/BE-0407-step-latency-driver-internal-tuning-ja.md)、
[BE-0234](../../roadmaps/BE-0234-adb-run-performance/BE-0234-adb-run-performance-ja.md)）。コードがすでに出荷された

...
調査したうえで対応しないと判断したときも、`実装済み` の項目は全チェックのチェックリストを保ちます。
そのユニットのチェックには印を付け、ユニット自身の説明を取り消し線で消し、理由をその下に入れ子の
サブバレットとして書き添えます。取り消し線は、コードの出荷ではなく判断によってそのユニットを閉じた
ことを示します（[BE-0407](../../roadmaps/BE-0407-step-latency-driver-internal-tuning/BE-0407-step-latency-driver-internal-tuning-ja.md)、
